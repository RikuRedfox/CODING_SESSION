...
void loveScale(){
    ...
    for (size_t i{0}; i < 13; i++){ // j for column
        for (size_t j{0}; j < 2; j++){
            ...
            std::cout << questions[i][j] << name; j++;
            std::cout << questions[i][j]; j++;
            ...
            while(1){
                ...
                try{
                    ...
                    std::cerr << "\nERROR: 1 to 5 only! Retry your input. ErrorCode : " << &x << "\n\n";
                }
            }
            // current scale + user scale
            current_scale += user_scale;
            // show the current points
            std::cout << "\nCurrent score: " << current_scale << "/" << total << std::endl << std::endl;
        }
    }
    // print the total/final point.
    std::cout << "The final score: " << current_scale << "/" << total << std::endl;
...
void likeScale(){
    ...
    for (size_t i{0}; i < 13; i++){ // j for column
        for (size_t j{0}; j < 2; j++){
            // call this array if the iteration met the condition
            if (questions[i][j] == questions[11][1] || questions[i][j] == questions[12][1]){
                std::cout << name << questions[i][j];
            }
            // increment j to call next column in same row
            std::cout << questions[i][j] << name; j++; 
...
int main(){
    ...
    if (stoi(respond) == 1){
        while (true){
            ...
            std::cout << "Type 'love' for love scale | 'like' for like scale\nRespond : ";
            std::cin >> respond;
            std::for_each(respond.begin(), respond.end(), [](char & c) {
                c = ::tolower(c);
            });
